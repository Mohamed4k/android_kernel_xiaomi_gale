...
int pd_hal_init_hardware(struct chg_alg_device *alg)
{
	...
	if (hal->chg2_dev)
		pr_debug("%s: Found secondary charger\n", __func__);
	else
		pr_debug("%s: Error : can't find secondary charger\n",
			__func__);

	hal->adapter = get_adapter_by_name("pd_adapter");
	if (hal->adapter)
		pr_debug("%s: Found pd adapter\n", __func__);
	else {
		pr_debug("%s: Error : can't find pd adapter\n",
			__func__);
		return -ENODEV;
	}
...
int pd_hal_get_uisoc(struct chg_alg_device *alg)
{
	...
	if (IS_ERR_OR_NULL(bat_psy)) {
		pr_debug("%s retry to get bat_psy\n", __func__);
