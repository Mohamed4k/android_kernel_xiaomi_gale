...
static void ehea_parse_eqe(struct ehea_adapter *adapter, u64 eqe)
{
	...
	case EHEA_EC_PORT_MALFUNC:
		netdev_dbg(dev, "Port malfunction\n");
		netif_carrier_off(dev);
		netif_tx_disable(dev);
		break;
	default:
...
out:
	if (ret)
		netdev_dbg(dev, "Failed starting. ret=%i\n", ret);
...
static int ehea_down(struct net_device *dev)
{
	...
	if (ret)
		netdev_dbg(dev, "Failed freeing resources. ret=%i\n", ret);
					 ...
					 struct device_node *dn)
{
	...
	netdev_dbg(dev, "Jumbo frames are %sabled\n",
		    jumbo == 1 ? "en" : "dis");
	...
	while ((eth_dn = of_get_next_child(lhea_dn, eth_dn))) {
		...
		adapter->port[i] = ehea_setup_single_port(adapter,
							  *dn_log_port_id,
							  eth_dn);
		if (adapter->port[i])
			netdev_dbg(adapter->port[i]->netdev,
				    "logical port id #%d\n", *dn_log_port_id);
		else
			       ...
			       const char *buf, size_t count)
{
	...
	if (port) {
		netdev_dbg(port->netdev, "adding port with logical port id=%d failed: port already configured\n",
			    logical_port_id);
		return -EINVAL;
	}
	...
	if (port) {
		...
		netdev_dbg(port->netdev, "added: (logical port id=%d)\n",
			    logical_port_id);
	} else {
		ehea_remove_adapter_mr(adapter);
		return -EIO;
				...
				const char *buf, size_t count)
{
	...
	if (port) {
		netdev_dbg(port->netdev, "removed: (logical port id=%d)\n",
			    logical_port_id);
