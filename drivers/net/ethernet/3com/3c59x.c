			 ...
			 int chip_idx, int card_idx)
{
	...
	if (dev->irq <= 0 || dev->irq >= nr_irqs)
		pr_debug(" *** Warning: IRQ %d is unlikely to work! ***\n",
			dev->irq);
	...
	if ((vp->available_media & 0x40) || (vci->drv_flags & HAS_NWAY) ||
		...
		mii_preamble_required--;
		if (phy_idx == 0) {
			pr_debug("  ***WARNING*** No MII transceivers found!\n");
			vp->phys[0] = 24;
		} else {
...
vortex_up(struct net_device *dev)
{
	...
	if (VORTEX_PCI(vp)) {
		...
		if (err) {
			pr_debug("%s: Could not enable device\n", dev->name);
			goto err_out;
		}
	}
...
vortex_error(struct net_device *dev, int status)
{
	...
	if (status & StatsFull) {			/* Empty statistics. */
		...
		if (DoneDidThat == 0  &&
			...
			pr_debug("%s: Updating statistics failed, disabling stats as an interrupt source\n",
				dev->name);
...
boomerang_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (vp->cur_tx - vp->dirty_tx >= TX_RING_SIZE) {
		if (vortex_debug > 0)
			pr_debug("%s: BUG! Tx Ring full, refusing to send buffer\n",
				dev->name);
		netif_stop_queue(dev);
		return NETDEV_TX_BUSY;
	}
...
_vortex_interrupt(int irq, struct net_device *dev)
{
	...
	do {
		...
		if (--work_done < 0) {
			pr_debug("%s: Too much work in interrupt, status %4.4x\n",
				dev->name, status);
	...
	while ((rx_status = ioread16(ioaddr + RxStatus)) > 0) {
		if (rx_status & 0x4000) { /* Error, update stats. */
			...
			if (skb != NULL) {
				...
				pr_debug("%s: No memory to allocate a sk_buff of size %d.\n",
					dev->name, pkt_len);
...
vortex_close(struct net_device *dev)
{
	...
	if (vp->rx_csumhits &&
		...
		pr_debug("%s supports hardware checksums, and we're not using them!\n",
			dev->name);
	}
#endif
