...
int m4u_sec_init(void)
{
	...
	if (!m4u_tee_en) {
		...
		pr_debug("[M4U] warning: reinit sec m4u en=%d\n", m4u_tee_en);
	}
m4u_sec_reinit:
	ret = __m4u_sec_init();
	if (ret < 0) {
...
irqreturn_t mtk_m4u_isr_sec(int irq, void *dev_id)
{
	...
	tf_en = res.a0;
	tf_port = res.a1;
	pr_debug("secure bank go back form secure world! en:%zu, fault_id:0x%lx\n",
		tf_en, tf_port);
...
static int m4u_probe(struct platform_device *pdev)
{
	...
	if (ret)
		...
		dev_dbg(m4u_dev->dev, "register fb_notifier OK!\n");
#endif

	dev_dbg(m4u_dev->dev, "%s done\n", __func__);
	return 0;
}
