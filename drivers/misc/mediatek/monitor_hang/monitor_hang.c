...
{
	pr_debug("[Hang_Detect] we  triger DB.\n");
...
static int __init monitor_hang_init(void)
{
	...
	if (unlikely(err)) {
		pr_debug("failed to register Hang_Monitor_dev device!\n");
		return err;
	}
	hang_detect_init();
