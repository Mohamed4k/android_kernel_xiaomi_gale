...
static int dlpt_adc_chan_init(struct platform_device *pdev)
{
	...
	if (ret) {
		if (ret != -EPROBE_DEFER)
			pr_debug("%s pmic_zcv fail, ret=%d\n", __func__, ret);
		return ret;
	}
...
static void dlpt_notify_init(void)
{
	...
	if (!dlpt.notify_ws)
		pr_debug("dlpt_notify_ws wakeup source fail\n");
...
static void pmic_uvlo_init(int uvlo_level)
{
	...
	default:
		val = 0;
		pr_debug("[dlpt] Invalid uvlo_level (%d)\n", uvlo_level);
		break;
	}
