...
static int mt6357_led_set_clock(struct led_classdev *cdev, int en)
{
	...
	dev_dbg(led->parent->dev, "%s: PMIC LED(%d) Set clock %s.\n",
		__func__, led->id, en ? "True" : "False");
...
static int mt6357_led_change_mode(struct led_classdev *cdev, int mode)
{
	...
	dev_dbg(led->parent->dev, "%s mode = %s\n",
				...
				unsigned long *delay_off)
{
	...
	dev_dbg(led->parent->dev, "%s: delay_on = %lu, delay_off=%lu\n",
	...
	dev_dbg(led->parent->dev, "%s: period = %lu\n", __func__, period);
	...
	duty /= precision;
	duty = duty - 1; //0~255
	dev_dbg(led->parent->dev, "%s: Duty = 0x%0x\n", __func__, duty);
	...
	dev_dbg(led->parent->dev, "%s: Frequency = 0x%0x\n", __func__, freq);
				     ...
				     struct device_node *np)
{
	...
	dev_dbg(led->parent->dev, "mt6357 parse led start\n");
...
static int mt6357_led_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "mt6357 led probe\n");
	...
	for_each_available_child_of_node(np, child) {
		...
		if (!ret)
			dev_dbg(dev, "read led 'step' property: %d\n", step);
		else
