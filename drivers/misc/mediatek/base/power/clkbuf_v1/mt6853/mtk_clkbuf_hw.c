...
int clk_buf_dts_map(void)
{
	...
	if (node)
		...
		pr_debug("%s can't find compatible node for pmif_spi\n",
			__func__);
		return -1;
	}
	...
	if (node)
		...
		pr_debug("%s can't find compatible node for pmif_spmi\n",
			__func__);
		return -1;
	}
	...
	if (node)
		...
		pr_debug("%s can't findcompatible node for spm_base_for_clk\n",
			__func__);
		return -1;
	}
