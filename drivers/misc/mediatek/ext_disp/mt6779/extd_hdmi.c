...
static void hdmi_resolution_setting(int arg)
{
	...
	if (hdmi_drv && hdmi_drv->get_params) {
		...
		if (dst_is_dsi) {
			...
			dpi = &extd_dpi_params.dispif_config.dpi;
			pdsc = &extd_dpi_params.dispif_config.dpi.dsc_params;

			dpi->clk_pol = hdmi_params->clk_pol;
			dpi->de_pol = hdmi_params->de_pol;
			dpi->hsync_pol = hdmi_params->hsync_pol;
			dpi->vsync_pol = hdmi_params->vsync_pol;

			dpi->hsync_pulse_width = hdmi_params->hsync_pulse_width;
			dpi->hsync_back_porch = hdmi_params->hsync_back_porch;
			dpi->hsync_front_porch = hdmi_params->hsync_front_porch;

			dpi->vsync_pulse_width = hdmi_params->vsync_pulse_width;
			dpi->vsync_back_porch = hdmi_params->vsync_back_porch;
			dpi->vsync_front_porch = hdmi_params->vsync_front_porch;

			dpi->dpi_clock = hdmi_params->input_clock;
			...
			dpi->width = p->hdmi_width;
			dpi->height = p->hdmi_height;
			if ((arg == HDMI_VIDEO_2160p_DSC_24Hz) ||
			    (arg == HDMI_VIDEO_2160p_DSC_30Hz)) {
				memset(pdsc, 0, sizeof(*pdsc));
				dpi->dsc_enable = 1;
				dpi->width = p->hdmi_width / 3;
				/* width/(slice_mode's slice) */
				pdsc->slice_width = 1920;
				pdsc->slice_hight = 8;
				/* 128: 1/3 compress; 192: 1/2 compress */
			...
			dpi->bg_width = p->bg_width;
			dpi->bg_height = p->bg_height;

			dpi->format = LCM_DPI_FORMAT_RGB888;
			dpi->rgb_order = LCM_COLOR_ORDER_RGB;
			dpi->i2x_en = true;
			dpi->i2x_edge = 2;
			dpi->embsync = false;
		}
