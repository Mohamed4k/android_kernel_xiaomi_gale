...
#include "sc202cs_sunny_marco_i_mipi_raw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "sc202cs_sunny_marco_i_mipi_raw"
...
extern unsigned char fusion_id_back2[96];
extern unsigned char sn_back2[96];

/*L19 code for HQ-161200 by TianGuchen at 2021/12/23 start*/
static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = SC202CS_SUNNY_MARCO_I_SENSOR_ID,
...
static kal_uint16 read_cmos_sensor_sc202cs(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
...
static void sc202cs_fusion_id_read(void)
{
	int i;
	for (i=0; i<8; i++) {
...
*************************************************************************/
  ...
  static kal_int8 sc202cs_sunny_read_general_info(int *addr, kal_uint16 addr_len, kal_uint8 *result)
  {
  	int i = 0;
  ...
  unsigned int sc202cs_sunny_read_otp(void)
  {
  	int ret = 0;
	kal_uint8 i;
	int otpdata_addr[] = {0x800A, 0x800B,0x8011,0x8012, 0x8000, 0x8001, 0x8002, 0x8003, 0x8004, 0x8005, 0x8006, 0x8007};
	ret = sc202cs_sunny_read_general_info(otpdata_addr, ARRAY_SIZE(otpdata_addr), otpData);
  	if (ret != 0) {
  ...
  unsigned int sc202cs_sunny_read_otp_info(struct i2c_client *client,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		pr_err("sunny return_sensor_id:0x%x\n",*sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {               
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                sc202cs_fusion_id_read();
                sc202cs_sn_read();      
                return ERROR_NONE;
            }   
...
UINT32 SC202CS_SUNNY_MARCO_I_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
