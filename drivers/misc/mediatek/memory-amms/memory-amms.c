...
static int __init amms_sysfs_init(void)
{
	...
	if (kobj) {
		if (sysfs_create_group(kobj, &attr_group)) {
			pr_debug("AMMS: sysfs create sysfs failed\n");
			return -ENOMEM;
		}
	} else {
		pr_debug("AMMS: Cannot find module %s object\n",
				KBUILD_MODNAME);
		return -EINVAL;
	}
				...
				phys_addr_t end_phys)
{
	...
	if (end_phys <= start_phys) {
		...
		pr_debug("%s end_phys is smaller than start_phys start_phys:%pa end_phys:%pa\n"
			, __func__, &start_phys, &end_phys);
		return -1;
	}

	if (!memblock_is_region_reserved(start_phys, end_phys - start_phys)) {
		pr_debug("%s:not reserved memory phys_start:%pa phys_end:%pa\n"
			, __func__, &start_phys, &end_phys);
		return -1;
	}
