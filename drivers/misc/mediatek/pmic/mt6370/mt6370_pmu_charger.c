...
static int __mt6370_chgdet_handler(struct mt6370_pmu_charger_data *chg_data)
{
	...
	dev_dbg(chg_data->dev, "%s chg_type:%d, pwr_rdy:%d\n",
...
static int mt6370_enable_power_path(struct charger_device *chg_dev, bool en)
{
	...
	dev_dbg(chg_data->dev, "%s: en = %d, pp_en = %d\n",
...
static void mt6370_get_bootmode(struct mt6370_pmu_charger_data *chg_data)
{
	...
	dev_dbg(chg_data->dev, "%s: size:%d tag:0x%X mode:%d type:%d\n",
				     ...
				     union power_supply_propval *val)
{
	...
	online = !online;
#endif
	dev_dbg(chg_data->dev, "%s: online = %d\n", __func__, online);
	val->intval = online;
					  ...
					  int min_uA, int max_uA)
{
	...
	dev_dbg(chg_data->dev, "%s: select otg_oc = %d\n",
