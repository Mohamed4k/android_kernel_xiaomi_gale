...
static void overflow_check(refcount_t *ref)
{
	...
	case REFCOUNT_SATURATED:
		...
		break;
	case REFCOUNT_MAX:
		pr_debug("Overflow detected: unsafely reset to max\n");
		break;
	default:
...
void lkdtm_REFCOUNT_INC_NOT_ZERO_OVERFLOW(void)
{
	...
	if (!refcount_inc_not_zero(&over))
		pr_debug("Weird: refcount_inc_not_zero() reported zero\n");
...
void lkdtm_REFCOUNT_ADD_NOT_ZERO_OVERFLOW(void)
{
	...
	if (!refcount_add_not_zero(6, &over))
		pr_debug("Weird: refcount_add_not_zero() reported zero\n");
...
static void check_zero(refcount_t *ref)
{
	...
	case REFCOUNT_SATURATED:
		...
		break;
	case REFCOUNT_MAX:
		pr_debug("Zero detected: unsafely reset to max\n");
		break;
	case 0:
		pr_debug("Still at zero: refcount_inc/add() must not inc-from-0\n");
		break;
	default:
...
static void check_negative(refcount_t *ref, int start)
{
	...
	if (refcount_read(ref) == start) {
		pr_debug("Still at %d: refcount_inc/add() must not inc-from-0\n",
			start);
		return;
	}
	...
	case REFCOUNT_SATURATED:
		...
		break;
	case REFCOUNT_MAX:
		pr_debug("Negative detected: unsafely reset to max\n");
		break;
	default:
		pr_err("Fail: refcount went crazy: %d\n", refcount_read(ref));
	}
...
void lkdtm_REFCOUNT_DEC_AND_TEST_NEGATIVE(void)
{
	...
	if (refcount_dec_and_test(&neg))
		pr_debug("Weird: refcount_dec_and_test() reported zero\n");
...
void lkdtm_REFCOUNT_SUB_AND_TEST_NEGATIVE(void)
{
	...
	if (refcount_sub_and_test(5, &neg))
		pr_debug("Weird: refcount_sub_and_test() reported zero\n");
...
static void check_saturated(refcount_t *ref)
{
	...
	case REFCOUNT_SATURATED:
		...
		break;
	case REFCOUNT_MAX:
		pr_debug("Saturation detected: unsafely reset to max\n");
		break;
	default:
		pr_err("Fail: refcount went crazy: %d\n", refcount_read(ref));
	}
