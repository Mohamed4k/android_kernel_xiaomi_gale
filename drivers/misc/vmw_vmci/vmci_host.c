...
int __init vmci_host_init(void)
{
	...
	if (IS_ERR(host_context)) {
		...
		pr_debug("Failed to initialize VMCIContext (error%d)\n",
			error);
		return error;
	}
	...
	if (error) {
		pr_debug("Module registration error (name=%s, major=%d, minor=%d, err=%d)\n",
			vmci_host_miscdev.name,
			MISC_MAJOR, vmci_host_miscdev.minor,
			error);
		pr_debug("Unable to initialize host personality\n");
